...
def _context(segments: list[tuple[float, float, str, str]], duration: float = 60.0):
    transcript = Transcript(
        segments=[
            TranscriptSegment(start, end, text, speaker) for start, end, speaker, text in segments
        ],
        backend="fixture",
    )
    turns = [SpeakerTurn(s.start, s.end, s.speaker or "SPEAKER_00") for s in transcript.segments]
    return JudgeContext(transcript=transcript, turns=turns, duration_seconds=duration, call_id="t")
...
def test_matching_pattern_scores_full_marks():
    rubric = parse_rubric(
        {"id": "r", "criteria": [{"id": "greeting", "patterns": [r"thanks for calling"]}]}
    )
...
def test_absent_pattern_scores_zero():
    rubric = parse_rubric(
        {"id": "r", "criteria": [{"id": "x", "patterns": [r"quantum entanglement"]}]}
    )
    result = score_transcript(rubric, _context(CONVERSATION))
...
def test_evidence_carries_timestamps_and_speaker():
    rubric = parse_rubric(
        {"id": "r", "criteria": [{"id": "resolution", "patterns": [r"no charge"]}]}
    )
...
def test_matching_is_case_insensitive():
    rubric = parse_rubric(
        {"id": "r", "criteria": [{"id": "g", "patterns": [r"THANKS FOR CALLING"]}]}
    )
...
def test_partial_max_score_scales_correctly():
    rubric = parse_rubric(
        {
            "id": "r",
            "criteria": [{"id": "a", "patterns": [r"thanks for calling"], "max_score": 5.0}],
        }
    )
    result = score_transcript(rubric, _context(CONVERSATION))
...
def test_llm_backend_builds_the_llm_judge():
    rubric = parse_rubric(
        {"id": "r", "judge": {"backend": "llm"}, "criteria": [{"id": "a", "patterns": ["x"]}]}
    )
...
def test_llm_judge_drops_malformed_evidence():
    ...
    rubric = parse_rubric(
        {"id": "r", "judge": {"backend": "llm"}, "criteria": [{"id": "a", "patterns": ["(?!)"]}]}
    )
    result = LlmJudge(rubric, client=client).judge(rubric.criteria[0], _context(CONVERSATION))
...
def test_build_prompt_is_pure():
    ...
    assert build_prompt(rubric.criteria[0], segments) == build_prompt(rubric.criteria[0], segments)
...
class _FakeResponse:
    def __init__(
        self, *, stop_reason: str, text: str | None = None, category: str | None = None
    ) -> None:
        self.stop_reason = stop_reason
        self.stop_details = types.SimpleNamespace(category=category) if category else None
        self.content = [types.SimpleNamespace(type="text", text=text)] if text is not None else []
    ...
    rubric = parse_rubric(
        {
            ...
            "criteria": [
                {
                    "id": "multi",
                    "patterns": [r"have a great day", r"thanks for calling"],
                }
            ],
        }
    )
